...
metal device list -p $METAL_PROJ_ID -o json | jq -r '.[] | .id + "\t" + .plan.slug + "\t" + .facility.code + "\t" + (.ip_addresses[]| select((.public==false) and .address_family==4) | .address|tostring) + "\t" + .hostname[0:15] + "\t\t" + .state[0:6] + "\t" +  .operating_system.slug[0:8]'
}

gtst_project_dashboard_nn(){
metal device list -p $METAL_PROJ_ID -o json | jq -r '.[] | .id + "\t" + .plan.slug + "\t" + .hostname[0:15] + "\t\t" + .state[0:6] + "\t" +  .operating_system.slug[0:8]'
}

gtst_instance_meta(){
INSTANCE_ID=$(metal -p $METAL_PROJ_ID device list -o json | jq --arg METAL_INSTANCE_NAME "$METAL_INSTANCE_NAME" -r '.[] | select(.hostname==$METAL_INSTANCE_NAME) | .id')
INSTANCE_BOND0=$(metal -p $METAL_PROJ_ID device list -o json | jq  --arg METAL_INSTANCE_NAME "$METAL_INSTANCE_NAME" -r '.[] | select(.hostname==$METAL_INSTANCE_NAME) | .network_ports[] | select(.name=="bond0") | .id')
